...
BUILTIN_TEMPLATE_NAMES = {row['模板名称'] for row in BUILTIN_TEMPLATES}


def get_default_config_data():
    """返回默认配置的字典，包含所有工作表数据（使用全局 BUILTIN_TEMPLATES）"""
    # 2D参数（37个）
    params_2d = [
...
class ConfigLoader:
    # 直接引用全局 BUILTIN_TEMPLATES，不再重复定义
    ...
    def _load_builtin_templates(self):
        """加载内置默认模板（使用全局 BUILTIN_TEMPLATES）"""
        self.template_def = []
        self.template_names = []
        self.template_id_rules = {}
