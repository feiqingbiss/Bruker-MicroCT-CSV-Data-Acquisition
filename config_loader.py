...
class ConfigLoader:
    ...
    def load(self, config_path=None):
        ...
        sheet_loaders = {
            ...
            'TemplateDef': self._load_template_def,
    ...
    def _load_template_def(self, xl):
...
def generate_default_config(path):
    ...
    with pd.ExcelWriter(path, engine='openpyxl') as writer:
        ...
        template_def_rows = [
            ...
            {'模板名称': '通用模板（一组样品不同部位、重复同名CSV）', '列名': '日期', '提取指令': 'DATE_META', '顺序': 1, '样品ID规则': ''},
        ...
        pd.DataFrame(template_def_rows).to_excel(writer, sheet_name='TemplateDef', index=False)
